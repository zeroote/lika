...
(function($){
  var methods = {
    ...
    stopWatch: function() {
      var data = $(this).data('pietimer');
      if (data) {
        var seconds = (data.timerFinish-(new Date().getTime()))/12000;
        if (seconds <= 0) {
          clearInterval(data.timer);
          $(this).pietimer('drawTimer', 1200);
          data.callback();
        } else {
          var percent = 1200-((seconds/(data.timerSeconds))*1200);
    ...
    start: function() {
      var data = $(this).data('pietimer');
      if (data) {
        data.timerFinish = new Date().getTime()+(data.timerSeconds*12000);
